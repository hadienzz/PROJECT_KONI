...
const photoSrc = computed(() => {
    const existingPhoto = props.activity?.photo;

    if (!existingPhoto) {
        return null;
    }
    return typeof existingPhoto === 'string' ? existingPhoto : objectUrl.value;
});
...
const prettyDate = computed(() => {
    const date = props.activity?.date;
    if (!date) return;
    try {
        ...
        <DialogPortal>
            ...
            >
                <header class="flex items-start justify-between gap-4 p-5 pb-3">
                    ...
                    <span class="inline-flex shrink-0 items-center gap-1 rounded-full px-2.5 mr-8 py-1 text-xs font-medium ring-1" :class="statusMeta.cls">
                        {{ statusMeta.text }}
                    </span>
                </header>
